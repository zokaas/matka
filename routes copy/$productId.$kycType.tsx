...
import { getMissingRequiredData, requiresApplicationUuid } from "~/utils";
import { PRODUCT } from "apps/kyc/config";
import { buildLoginRedirectUrl } from "~/utils/urlBuilder.server";
...
export const loader = async ({
    ...
    const applicationUuid = session.get("applicationUuid") ?? "";
    const kycFlow = session.get("kycFlow") ?? "";
    ...
    try {
        ...
        const applicationReference =
            productId === PRODUCT.SWEDEN_B2B ? applicationUuid : applicationId;

        const loginRedirectFullUrl = buildLoginRedirectUrl(
            productId,
            kycFlow,
            loginUrl,
        ...
        loaderData.sessionData = {
            ...
            exp,
            loginUrl: loginRedirectFullUrl,
        };

        const maxAge = computeMaxAgeFromExp(exp / 1000);
        session.set("loginUrl", loginRedirectFullUrl);
...
export const action: ActionFunction = async ({ request }: ActionFunctionArgs) => {
    try {
        ...
        const { applicationUuid, kycDoneUrl, applicationId, clientId, kycType } = getSessionProps(
            session,
            "applicationUuid",
            "kycDoneUrl",
            "applicationId",
        ...
        const mustHaveApplicationUuid = requiresApplicationUuid(clientId);

        const { missing, hasMissingRequiredData } = getMissingRequiredData({
            answersJson,
            applicationId,
            applicationUuid,
        ...
        if (hasMissingRequiredData) {
            ...
            return {
                ...
                error: {
                    ...missing,
                    applicationUuidRequired: mustHaveApplicationUuid,
                },
            };
        }

        if (!kcUserId || !authData || !answersJson || !questionSetId || !applicationId) {
            throw new Error("Unreachable: missing data already validated");
        }
        ...
        const result = await sendFormData(payload, clientId, kycType, applicationId, sessionId);
