...
export const FormPage: React.FC<T_FormPageProps> = (props: T_FormPageProps) => {
    ...
    const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        //clean up answers for automatic analysis, so that if question is not showed to customer it is not sent to cm1
        //TODO: maybe move or change logic in future?
        const cleanQuestionsForAutomaticAnalysis = new Map(formValues);
        cleanQuestionsForAutomaticAnalysis.forEach((answerObj, key) => {
            if (!isVisible(key, formValuesMap)) {
                cleanQuestionsForAutomaticAnalysis.set(key, {
                    ...answerObj,
                    automaticAnalysis: false,
                });
            }
        });
        submitFormAnswers(cleanQuestionsForAutomaticAnalysis, String(formData.id), submit);
    };
        ...
        <main className={mainContentStyle}>
            <Container className={mainContainerStyle}>
                ...
                <Form ref={formRef} method="post" onSubmit={handleFormSubmit}>
                    {/* Progress Steps */}
                    <Steps steps={formData.generalFormData.steps} activeStep={activeStep} />
                    {/* Divider */}
