  ...
  private cleanQuestions(
    ...
    for (const questionComponent of setOfQuestions) {
      try {
        ...
        if (!attrs) {
          this.logger.warn(`No attributes for question ${questionComponent.id} in ${lang}`);
          continue;
        }
        ...
        this.logger.debug(`Cleaned question ${questionComponent.id}: ${attrs.questionParameter}`);
      } catch (error) {
