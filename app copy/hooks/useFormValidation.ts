...
import { validateField, createValidationConfig, isFieldVisible } from "~/utils/validation";
...
export const useFormValidation = (formData: T_ParsedFormData) => {
    ...
    const validateSingleField = useCallback(
        (fieldName: string, value: T_AnswerValue, updateState = true): boolean => {
    ...
    const validateEntireForm = useCallback(
        (formValues: Map<string, T_AnswerValue>): T_FormValidationResult => {
    ...
    const checkIsVisible = useCallback(
        (fieldName: string, formValues: Map<string, T_AnswerValue>): boolean => {
