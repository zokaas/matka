...
import { ThankYouState } from "./thankyouPage.types";
...
const ThankYouPage: React.FC = () => {
    ...
    const rawPipelineApplication = useAppSelector((s) => s.applicationResponse.pipelineApplication);

    const thankYouState: ThankYouState | null = useMemo(() => {
        if (status === flowStatus.ONBOARDING) {
            if (!rawPipelineApplication) return null;

            return {
                status,
                pipelineApplication: rawPipelineApplication,
            };
        }

        return { status };
    }, [status, rawPipelineApplication]);

    const showKycButton = isKYCServiceEnabled() && status === flowStatus.ONBOARDING;

    console.log("isKYCServiceEnabled", isKYCServiceEnabled());
    console.log("showKycButton", showKycButton);

    useEffect(() => {
        if (!thankYouState) navigate(E_Routes.ERROR);
    }, [thankYouState, navigate]);

    if (!thankYouState) return null;
    ...
    const handleStartKyc = useCallback(async () => {
        if (thankYouState.status !== flowStatus.ONBOARDING) return;

        const { applicationId, applicationUuid } = thankYouState.pipelineApplication;
        ...
        try {
            await startKyc(dispatch, dataForKyc);
        } catch (err) {
