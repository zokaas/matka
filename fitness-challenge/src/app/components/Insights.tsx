...
useEffect(() => {
  const fetchData = async () => {
    try {
      ...
      // Calculate achievements & insights after targetPaces is set
      const calculatedTargetPaces = calculateTargetPaces(data);
      console.log("Calculated Target Paces:", calculatedTargetPaces); // Debug log
      setTargetPaces(calculatedTargetPaces);

      calculateAchievements(data);
      calculateWeeklyInsights(data, calculatedTargetPaces); // Pass correct targetPaces
    } catch (err) {
...
console.log(
  `Remaining Distance: ${remainingDistance}, Weeks Remaining: ${weeksRemaining}`
);
console.log(`Weekly Goal Per User: ${requiredPerUser / weeksRemaining}`);
const weeklyPerUser =
...
const calculateWeeklyInsights = (users: User[], targetPaces: TargetPaces) => {
  const today = new Date();
  const weekAgo = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000);

  if (!targetPaces) {
    console.error("Target paces not available!");
    return;
  }

  const weeklyGoal = targetPaces.weeklyPerUser; // Use passed-in targetPaces
  console.log(`Weekly Goal Per User: ${weeklyGoal}`);
  ...
  console.log("Updated Weekly Insights:", sortedInsights);
  ...
  const getLastFourWeeks = () => {
    ...
    const allSports = new Set(
      weeks.flatMap((w) => w.sports.map((s) => s.name))
    );
      ...
      comparisons: weeks.slice(1).map((week, i) => ({
        weekChange:
          weeks[i].kilometers === 0
            ? 0
            : Math.round(
                ((week.kilometers - weeks[i].kilometers) /
                  weeks[i].kilometers) *
                  100
              ),
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="bg-orange-50 p-4 rounded-lg">
              ...
              {(() => {
                ...
                const thisWeekKm = users.reduce((sum, user) => {
                  return (
                    sum +
                    user.activities
                      .filter((a) => new Date(a.date) >= monday)
                      .reduce((kmSum, a) => kmSum + a.kilometers, 0)
                  );
                }, 0);

                const weeklyTeamTarget =
                  targetPaces.weeklyPerUser * users.length;
                const progressPercentage = Math.min(
                  100,
                  (thisWeekKm / weeklyTeamTarget) * 100
                );
                ...
                return (
                  <>
                    ...
                    <div className="flex justify-between text-sm text-orange-600">
                      <span>{thisWeekKm.toFixed(1)} km</span>
                      <span>({progressPercentage.toFixed(0)}%)</span>
                    </div>
                    <p className="text-sm text-orange-600 mt-1">
                      Tavoite:{" "}
                      {Math.round(weeklyTeamTarget).toLocaleString("fi-FI")} km
                      yhteensä
                    </p>
                  </>
                );
            ...
            <InsightAlert
              ...
              description={
                <>
                  Tavoitevauhti:{" "}
                  <strong>
                    {Math.round(targetPaces.weeklyPerUser)} km/vko
                  </strong>{" "}
                  per hlö.
                  <br />
            ...
            <LineChart data={getTargetLine()}>
              ...
              <Tooltip
                formatter={(value: number) =>
                  value
                    ? `${Math.round(value).toLocaleString("fi-FI")} km`
                    : "-"
                }
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse rounded-xl overflow-hidden shadow-sm">
                <thead className="bg-purple-100 text-purple-800 sticky top-0">
                  <tr>
                    {["Petollinen", "Viikon edistyminen", "Loppuviikon päiväkohtainen tavoite"].map(
                      (heading) => (
                        <th
                          key={heading}
