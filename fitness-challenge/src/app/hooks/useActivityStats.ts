    ...
    const fetchStats = async () => {
      try {
        ...
        for (const user of users) {
          ...
          // Filter only challenge-period activities
          const start = new Date(challengeParams.startDate);
          const end = new Date(challengeParams.endDate);
        ...
        // Global stats
        if (isGlobal) {
          const maxKm = Math.max(...allStats.map((s) => s.bestKmDay?.kilometers || 0));
          const maxWorkout = Math.max(...allStats.map((s) => s.longestWorkoutDay?.duration || 0));
          const maxStreak = Math.max(...allStats.map((s) => s.currentStreak.days));

          setStats({
            bestKmDay: allStats.find((s) => s.bestKmDay?.kilometers === maxKm)?.bestKmDay,
            longestWorkoutDay: allStats.find((s) => s.longestWorkoutDay?.duration === maxWorkout)?.longestWorkoutDay,
            currentStreak: {
              start: allStats.find((s) => s.currentStreak.days === maxStreak)?.currentStreak.start || "",
              days: maxStreak,
            },
          });
        } else {
