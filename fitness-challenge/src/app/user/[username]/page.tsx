...
exit={{ opacity: 0, y: -50 }}
className={`fixed top-4 right-4 z-50 p-4 rounded-lg shadow-lg flex items-center gap-3 min-w-80 ${ type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : type === 'error' ? 'bg-red-50 text-red-800 border border-red-200' : 'bg-blue-50 text-blue-800 border border-blue-200' }`}
...
```
// Check if this is a custom activity in format "CustomSport / Muu(...)"
const customMatch = activity.activity.match(/^(.+?)\s*\/\s*(Muu.*)$/i);
if (customMatch) {
