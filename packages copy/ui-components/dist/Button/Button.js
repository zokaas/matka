...
export const Button = ({ onClick, disabled = false, children, variant = "neutral", className = "", type = "button", }) => {
    const variantClasses = variantClassMap[variant] || variantClassMap.neutral;
    const disabledClasses = disabled ? "opacity-50 cursor-not-allowed" : "";
    return (_jsx("button", { type: type, onClick: onClick, disabled: disabled, className: `${baseClasses} ${variantClasses} ${disabledClasses} ${className}`, children: children }));
};
